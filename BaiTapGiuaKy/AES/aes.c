#include "aes.h"
#include <string.h>

#define Nb 4      // so cot cua state
#define Nk 4      // AES-128: key 16 byte => 4 tu
#define Nr 10     // so round cho AES-128

// Bang S-box (256 gia tri)
static const uint8_t sbox[256] = {
    
};

// Bang inverse S-box
static const uint8_t inv_sbox[256] = {
    
};

// Mang Rcon (cho key expansion), 11 gia tri de AES-128
static const uint8_t Rcon[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36};

// Ham KeyExpansion: mo rong key 16 byte thanh mang expandedKey 176 byte
static void KeyExpansion(const uint8_t key[16], uint8_t expandedKey[176]) {
    memcpy(expandedKey, key, 16);
    int bytesGenerated = 16;
    int rconIteration = 1;
    uint8_t temp[4];
    
    while(bytesGenerated < 176) {
        for (int i = 0; i < 4; i++) {
            temp[i] = expandedKey[bytesGenerated - 4 + i];
        }
        if (bytesGenerated % 16 == 0) {
            // RotWord: xoay vong 4 byte
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;
            // SubWord: thay the tung byte bang S-box
            for (int i = 0; i < 4; i++)
                temp[i] = sbox[temp[i]];
            // Rcon
            temp[0] ^= Rcon[rconIteration];
            rconIteration++;
        }
        for (int i = 0; i < 4; i++) {
            expandedKey[bytesGenerated] = expandedKey[bytesGenerated - 16] ^ temp[i];
            bytesGenerated++;
        }
    }
}

static void AddRoundKey(uint8_t state[16], const uint8_t roundKey[16]) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

static void SubBytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

static void InvSubBytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) {
        state[i] = inv_sbox[state[i]];
    }
}

static void ShiftRows(uint8_t state[16]) {
    uint8_t temp[16];
    memcpy(temp, state, 16);
    // Hang 0 khong doi
    state[0]  = temp[0];  state[4]  = temp[4];  state[8]  = temp[8];  state[12] = temp[12];
    // Hang 1 dich trai 1 vi tri
    state[1]  = temp[5];  state[5]  = temp[9];  state[9]  = temp[13]; state[13] = temp[1];
    // Hang 2 dich trai 2 vi tri
    state[2]  = temp[10]; state[6]  = temp[14]; state[10] = temp[2];  state[14] = temp[6];
    // Hang 3 dich trai 3 vi tri
    state[3]  = temp[15]; state[7]  = temp[3];  state[11] = temp[7];  state[15] = temp[11];
}

static void InvShiftRows(uint8_t state[16]) {
    uint8_t temp[16];
    memcpy(temp, state, 16);
    // Hang 0 khong doi
    state[0]  = temp[0];  state[4]  = temp[4];  state[8]  = temp[8];  state[12] = temp[12];
    // Hang 1 dich phai 1 vi tri
    state[1]  = temp[13]; state[5]  = temp[1];  state[9]  = temp[5];  state[13] = temp[9];
    // Hang 2 dich phai 2 vi tri
    state[2]  = temp[10]; state[6]  = temp[14]; state[10] = temp[2];  state[14] = temp[6];
    // Hang 3 dich phai 3 vi tri
    state[3]  = temp[7];  state[7]  = temp[11]; state[11] = temp[15]; state[15] = temp[3];
}

static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ (((x >> 7) & 1) ? 0x1b : 0));
}

static void MixColumns(uint8_t state[16]) {
    for (int i = 0; i < 4; i++) {
        int col = i * 4;
        uint8_t a0 = state[col];
        uint8_t a1 = state[col + 1];
        uint8_t a2 = state[col + 2];
        uint8_t a3 = state[col + 3];
        uint8_t r0 = xtime(a0) ^ (a1 ^ xtime(a1)) ^ a2 ^ a3;
        uint8_t r1 = a0 ^ xtime(a1) ^ (a2 ^ xtime(a2)) ^ a3;
        uint8_t r2 = a0 ^ a1 ^ xtime(a2) ^ (a3 ^ xtime(a3));
        uint8_t r3 = (a0 ^ xtime(a0)) ^ a1 ^ a2 ^ xtime(a3);
        state[col]     = r0;
        state[col + 1] = r1;
        state[col + 2] = r2;
        state[col + 3] = r3;
    }
}

static uint8_t mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1)
            p ^= a;
        uint8_t hi_bit = a & 0x80;
        a <<= 1;
        if (hi_bit)
            a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

static void InvMixColumns(uint8_t state[16]) {
    for (int i = 0; i < 4; i++) {
        int col = i * 4;
        uint8_t a0 = state[col];
        uint8_t a1 = state[col + 1];
        uint8_t a2 = state[col + 2];
        uint8_t a3 = state[col + 3];
        state[col]     = mul(0x0e, a0) ^ mul(0x0b, a1) ^ mul(0x0d, a2) ^ mul(0x09, a3);
        state[col + 1] = mul(0x09, a0) ^ mul(0x0e, a1) ^ mul(0x0b, a2) ^ mul(0x0d, a3);
        state[col + 2] = mul(0x0d, a0) ^ mul(0x09, a1) ^ mul(0x0e, a2) ^ mul(0x0b, a3);
        state[col + 3] = mul(0x0b, a0) ^ mul(0x0d, a1) ^ mul(0x09, a2) ^ mul(0x0e, a3);
    }
}

static void Cipher(uint8_t state[16], const uint8_t expandedKey[176]) {
    AddRoundKey(state, expandedKey);
    for (int round = 1; round < Nr; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, expandedKey + round * 16);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, expandedKey + Nr * 16);
}

static void InvCipher(uint8_t state[16], const uint8_t expandedKey[176]) {
    AddRoundKey(state, expandedKey + Nr * 16);
    for (int round = Nr - 1; round >= 1; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, expandedKey + round * 16);
        InvMixColumns(state);
    }
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, expandedKey);
}

void aes_encrypt_block(const uint8_t in[16], uint8_t out[16], const uint8_t key[16]) {
    uint8_t state[16];
    memcpy(state, in, 16);
    uint8_t expandedKey[176];
    KeyExpansion(key, expandedKey);
    Cipher(state, expandedKey);
    memcpy(out, state, 16);
}

void aes_decrypt_block(const uint8_t in[16], uint8_t out[16], const uint8_t key[16]) {
    uint8_t state[16];
    memcpy(state, in, 16);
    uint8_t expandedKey[176];
    KeyExpansion(key, expandedKey);
    InvCipher(state, expandedKey);
    memcpy(out, state, 16);
}
